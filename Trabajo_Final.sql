...
-- ALTA TICKET
DELIMITER $$
CREATE TRIGGER tr_insert_ticket
AFTER INSERT ON tickets
FOR EACH ROW
BEGIN
INSERT INTO log_ticket (event_name, ID, operador_nuevo, severidad_nuevo, estado_nuevo, grupo_de_derivacion_nuevo, usuario, fecha) 
VALUES ('Alta ticket', new.ID, NEW.id_operador, NEW.id_severidad, NEW.id_estado, NEW.id_grupo_de_derivacion, USER(), NOW());
END$$
...
INSERT INTO empresas (empresa) VALUES ('PESPI');
INSERT INTO empresas (empresa) VALUES ('BURGER KING');
...
, titulo
, descripcion) 
VALUES ( 1, 2, 3, 1, 1, 1, 3, 'titulo', 'descripcion del caso');
...
, titulo
, descripcion) 
VALUES ( 4, 2, 6, 1, 1, 1, 3, 'Alta ticket', 'Prueba alta ticket');
...
, titulo
, descripcion) 
VALUES ( 1, 2, 1, 1, 1, 1, 3, 'Titulo ticket 2', 'basta de tickets');
...
DELIMITER $$
CREATE PROCEDURE SP_SolucionarComentario( IN ID int, IN id_ticket int
)
BEGIN
START TRANSACTION;
    ...
    IF @id_comentario >= 1 THEN
		...
		WHERE c.ID = @id_comentario AND c.id_ticket = @idticket;
	ELSE 
		ROLLBACK;
	END IF;
COMMIT;
...
BEGIN
	...
	call SP_SolucionarComentario(idcomentario, idticket);
...
BEGIN
		DECLARE operador, contacto, empresa, severidad, grupo_de_derivacion INT;
					...
					IF empresa = 1 THEN
                        ...
                        IF severidad = 1 THEN
							...
							IF grupo_de_derivacion = 1 THEN
								INSERT INTO tickets (id_operador, id_contacto, id_empresa, id_severidad, id_categoria, id_grupo_de_derivacion, titulo, descripcion)
								VALUES (@id_operador, @id_contacto, @id_empresa, @id_severidad, @id_categoria, @id_grupo_de_derivacion, @titulo, @descripcion);
							END IF;
						END IF;
					END IF;
				END IF;
...
DELIMITER $$
CREATE PROCEDURE SP_Closed_ticket(IN in_id INT)
BEGIN
    DECLARE Ticket INT;
...
BEGIN
    ...
    DECLARE mensaje text default 'Operacion completada';
	START TRANSACTION;
    SELECT p.ID INTO idpais FROM paises as p where UPPER(in_pais) = p.pais;
    IF idpais != 0 THEN
		INSERT INTO empresas (empresa) VALUES (UPPER(in_nombre));
